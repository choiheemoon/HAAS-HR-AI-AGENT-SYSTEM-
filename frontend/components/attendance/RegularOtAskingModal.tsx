...
import { OT_TYPES, apiItemToRow, newRowTemplate, normalizeTimeInput, type OtBufferRow } from '@/lib/additionalOtModel';
...
export default function RegularOtAskingModal({
  open,
  onClose,
  employee,
  inquiryDateFrom,
  ...
  const [formRow, setFormRow] = useState<OtBufferRow | null>(null);
  ...
  const otInputCls = 'w-full border border-gray-300 rounded px-1.5 py-0.5 text-center text-xs font-mono tabular-nums bg-white';
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="regular-ot-asking-title"
      onMouseDown={(e) => {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-2 px-2.5 py-1.5 border-b border-gray-200 bg-gradient-to-r from-sky-50 to-white shrink-0">
          <h2 id="regular-ot-asking-title" className="text-sm font-bold text-sky-950 pr-2">
            {t('additionalOt.regularOtAskingTitle')}
          </h2>
          <button
            type="button"
        ...
        <div className="flex-1 min-h-0 overflow-hidden p-2 flex flex-col gap-1.5">
          ...
          <div className="flex flex-wrap items-end gap-x-2 gap-y-1 border border-gray-200 rounded-md bg-white/90 p-1.5 shrink-0">
            ...
            <div className="min-w-0 flex-1 basis-[14rem]">
              ...
              <div
                ...
                title={`${employee.employee_number} · ${employee.name} · ${employee.swipe_card || '-'} · ${employee.id}`}
              >
            ...
            <label className="text-[10px] text-gray-600 shrink-0 flex flex-col gap-0.5">
              ...
              <input
                type="date"
                className={cn(inputCls, 'min-w-[8.5rem]')}
                value={dateFrom}
          ...
          {formRow ? (
            <div className="rounded-md border border-sky-200 bg-sky-50/60 p-1.5 space-y-1.5 shrink-0">
              <div className="text-[11px] font-semibold text-sky-950">{t('additionalOt.title')}</div>
              <div className="grid grid-cols-2 sm:grid-cols-6 gap-1">
                <label className="text-[10px] text-gray-700">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.date')}</span>
                  <input type="date" className={inputCls} value={formRow.work_date} onChange={(e) => patchForm({ work_date: e.target.value })} />
                </label>
                <label className="text-[10px] text-gray-700 sm:col-span-2">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.otType')}</span>
                  <select className={inputCls} value={formRow.ot_type} onChange={(e) => patchForm({ ot_type: e.target.value })}>
                    {OT_TYPES.map((opt) => (
                ...
                <label className="text-[10px] text-gray-700">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.otStart')}</span>
                  <input
                    type="text"
                    className={otInputCls}
                    value={formRow.ot_start}
                    onChange={(e) => patchForm({ ot_start: e.target.value })}
                    onBlur={() => {
                      const v = normalizeTimeInput(formRow.ot_start);
                      if (v || formRow.ot_start.trim() === '') patchForm({ ot_start: v });
                    }}
                  />
                </label>
                <label className="text-[10px] text-gray-700">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.otEnd')}</span>
                  <input
                    type="text"
                    className={otInputCls}
                    value={formRow.ot_end}
                    onChange={(e) => patchForm({ ot_end: e.target.value })}
                    onBlur={() => {
                      const v = normalizeTimeInput(formRow.ot_end);
                      if (v || formRow.ot_end.trim() === '') patchForm({ ot_end: v });
                    }}
                  />
                </label>
                <label className="text-[10px] text-gray-700">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.typeOt')}</span>
                  <input type="text" className={inputCls} value={formRow.type_ot} onChange={(e) => patchForm({ type_ot: e.target.value })} />
                </label>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-6 gap-1">
                <label className="text-[10px] text-gray-700">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.jobTitle')}</span>
                  <input
                    type="number"
                    className={inputCls}
                    value={formRow.job_title_code}
                    onChange={(e) => patchForm({ job_title_code: parseInt(e.target.value, 10) || 0 })}
                  />
                </label>
                <label className="text-[10px] text-gray-700">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.breakType')}</span>
                  <input
                    type="number"
                    className={inputCls}
                    value={formRow.ot_breaktime_type}
                    onChange={(e) => patchForm({ ot_breaktime_type: parseInt(e.target.value, 10) || 0 })}
                  />
                </label>
                <label className="text-[10px] text-gray-700">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.blockPay')}</span>
                  <div className="h-[1.75rem] px-2 border border-gray-300 rounded bg-white flex items-center">
                    <input type="checkbox" checked={formRow.block_payment} onChange={(e) => patchForm({ block_payment: e.target.checked })} />
                  </div>
                </label>
                <label className="text-[10px] text-gray-700 sm:col-span-2">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.approve')}</span>
                  <input type="text" className={inputCls} value={formRow.approve_status} onChange={(e) => patchForm({ approve_status: e.target.value })} />
                </label>
                <label className="text-[10px] text-gray-700 sm:col-span-6">
                  <span className="block leading-none mb-0.5">{t('additionalOt.col.note')}</span>
                  <input type="text" className={inputCls} value={formRow.note} onChange={(e) => patchForm({ note: e.target.value })} />
                </label>
              </div>
            </div>
          ) : null}

          <p className="text-[10px] text-gray-500 px-0.5 leading-snug shrink-0 line-clamp-2">{t('additionalOt.gridHint')}</p>
